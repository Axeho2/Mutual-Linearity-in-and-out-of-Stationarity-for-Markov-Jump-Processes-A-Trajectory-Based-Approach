...
This is the positive-correlation version for arbitrary N.

Model
-----
Open N-site SEP with hard-core exclusion:
- each site can be empty or occupied by at most one particle
- left reservoir exchanges particles with site 1
- right reservoir exchanges particles with site N
- particles hop between neighboring sites

Theorem-compatible choices
--------------------------
...
- Q2 is the dwelling time in the fully occupied state 11...1

As the left-boundary injection rate lambda increases, both the right-reservoir
current and the time spent in the fully occupied configuration typically increase,
leading to a positive slope in the parametric plot of Q1-hat versus Q2-hat.
"""
...
def plot_panel(ax, data):
    ...
    ax.set_xlim(*xlim)
    ax.set_ylim(*ylim)
    ax.set_xlabel(r"$\hat{Q}_2(\omega)$")
...
def main():
    ...
    pdf_path = outdir / f"sep{N_SITES}_positive.pdf"
    svg_path = outdir / f"sep{N_SITES}_positive.svg"
